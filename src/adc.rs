    ...
    ($ADC:ident, $ADC_COMMON:ident, $adc:ident, $rcc_num:tt) => {
        impl Adc<pac::$ADC> {
            ...
            pub fn enable(&mut self) {
                ...
                self.regs.isr().modify(|_, w| w.adrdy().clear());
                // 2. Set ADEN=1.
            ...
            pub fn clear_interrupt(&mut self, interrupt: AdcInterrupt) {
                self.regs.isr().write(|w| match interrupt {
                    AdcInterrupt::Ready => w.adrdy().clear(),
                    AdcInterrupt::EndOfConversion => w.eoc().clear(),
                    AdcInterrupt::EndOfSequence => w.eos().clear(),
                    AdcInterrupt::EndofConversionInjected => w.jeoc().clear(),
                    AdcInterrupt::EndOfSequenceInjected => w.jeos().clear(),
                    AdcInterrupt::Watchdog1 => w.awd1().clear(),
                    AdcInterrupt::Watchdog2 => w.awd2().clear(),
                    AdcInterrupt::Watchdog3 => w.awd3().clear(),
                    AdcInterrupt::EndOfSamplingPhase => w.eosmp().clear(),
                    AdcInterrupt::Overrun => w.ovr().clear(),
                    AdcInterrupt::InjectedOverflow => w.jqovf().clear(),
                });
                // match interrupt {
